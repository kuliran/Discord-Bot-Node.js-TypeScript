...
exports.default = () => {
    const systemFolderPaths = getAllFiles(path.join(__dirname, "..", "systems"), true);
    const eventPaths = {};
    const commandPaths = [];
    ...
    if (LOG_FETCH_RESULT) {
        console.log(`[pathFinder]: logging fetch result. You can turn this off here: ${__filename}`);
        console.log("events:", eventPaths);
        console.log("commands", commandPaths);
    }
    return { eventPaths, commandPaths };
};
function getAllFiles(directory, foldersOnly = false, includeDescendants = false) {
    ...
    for (const file of readFiles) {
        ...
        if (file.isDirectory()) {
            ...
            if (includeDescendants) {
                filePathNames = filePathNames.concat(getAllFiles(fullPathName, foldersOnly, true));
            }
        }
        else if (file.isFile()) {
            if (!foldersOnly) {
